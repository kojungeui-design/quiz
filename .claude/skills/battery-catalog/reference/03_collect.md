...
```python
CCA_RE = re.compile(
    r'(?:CCA|cold\s*crank\w*)\s*[:\-]?\s*(\d{3,4})'                  # CCA 750
    r'|(\d{3,4})\s*(?:CCA\b|A(?![a-z]))\s*\(?(EN|SAE|DIN)?\)?', re.I)  # 750 CCA / 750A (EN)
```

**`A(?![a-z])` 의 부정 선읽기가 핵심이다.** 없으면 `100Ah` 를 CCA 100 으로 읽는다.
그리고 **`750 CCA` 처럼 숫자가 먼저 오는 표기를 빠뜨리지 마라** — 현장에선
이쪽이 더 흔하다.

**규격코드는 정규식이 아니라 목록으로 대조해라.** `group size 24` 같은 키워드를
요구하면 안전하긴 한데 **카달로그 표를 통째로 놓친다.** 표에는 그 단어가 없고
`S4 24F-525B  24F  525  650` 처럼 코드만 온다. 실재하는 BCI 그룹 목록을 두고,

- 글자 붙은 코드(`24F` `94R` `8D` `H6`) → 바로 받는다
- 맨숫자(`24` `65`) → **한 줄에 두 번 나오거나 품번꼴(`65-750B`)일 때만.**
  안 그러면 `Table 27 shows results for 2023` 이 BCI 27 이 된다

JIS 도 `75D23L` 뿐 아니라 **`D23` 처럼 그룹만 오는 표기**를 받아야 한다.
