...
export function whatIfCard(ctx, plan) {
  ...
  let index = 0;
  const build = () => {
    mount.replaceChildren(designCard(ctx, plan, tunable[index], tunable, index, (next) => { index = next; build(); }));
  };
  build();
  return mount;
}
 ...
 * @param {object} ctx
 * @param {object} plan   현재 선택안
 * @param {object} baseline 조절 전 설계 (plan.designs 중 하나)
 * @param {object[]} tunable 조절 가능한 설계 목록
 * @param {number} index    baseline 의 위치
 * @param {(i:number)=>void} onPick 제품 변경
 */
function designCard(ctx, plan, baseline, tunable, index, onPick) {
  const { engine, project } = ctx;
  const spec = project.lineup.find((item) => item.uid === baseline.spec.uid) || baseline.spec;
  // 조절 기준값 — 조절 전 설계가 실제로 쓴 값
  const basePosT = Number.parseFloat(String(baseline.posThickness).replace(/[^0-9.]/g, '')) || 0.7;
  const baseNegT = Number.parseFloat(String(baseline.negThickness).replace(/[^0-9.]/g, '')) || 0.7;
  const state = {
    posThickness: basePosT,
    negThickness: baseNegT,
    posActiveWeight: baseline.posActiveWeight,
    plateCount: baseline.plateCount,
  };
  const baseValues = { ...state };
